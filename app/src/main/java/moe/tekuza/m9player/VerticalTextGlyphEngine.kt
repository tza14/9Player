...
internal object VerticalTextGlyphEngine {
    ...
    private val rotateClockwise = setOf(
        ...
        '〖', '〗', '＜', '＞', '〜', '～', '…', '‥', '-', '_', '~',
        '／', '/', '｜', '|', '＝', '=', '÷', '：', ':', '；', ';',
        // 縦組专用の双引号（始め〝 / 終わり〟）：字体给的是未转向的字面（实测笔画 `\`、两笔上下对齐，
        // 且落在字框的另一组角上），而 JLREQ 把这两个字归在「字面の向きを変更する」的括弧类里
        // （cl-01 / cl-02，fig2_6）。它们没有 presentation form，所以走旋转这条现成路：
        // 实测 +90°（顺时针）后朝向 `/`、落角（〝 右下 / 〟 左上）与 fig2_6 一致。
        '〝', '〟'
    )
    ...
    private val noColumnStartChars: Set<Char> = setOf(
        ...
        '︑', '︒', '︐', '︓', '︔', '︕', '︖', '︶', '︺', '︸', '﹀',
        '︙', '︰', '﹂', '﹄', '﹡',
        // 終わりダブルミニュート＝終わり括弧類（JLREQ cl-02，縦組専用）→ 列頭禁則
        '〟'
    )
    ...
    private val noColumnEndChars: Set<Char> = setOf(
        '「', '『', '“', '‘', '（', '(', '[', '{', '【', '〔', '〈', '《', '＜',
        '︵', '︹', '︷', '︿', '﹁', '﹃',
        // 始めダブルミニュート＝始め括弧類（JLREQ cl-01，縦組専用）→ 列末禁則
        '〝'
    )
